...
struct link_change {
	float time;   /* The time when the change happens */
	int node_a;   /* One of the nodes on the link to be changed */
	int node_b;   /* The other node on the link to be changed. */
	int new_cost; /* The new cost of the link */
};
...
//int connectcosts[NUMOFNODES][NUMOFNODES]={{0,4,50},{4,0,1},{50,1,0}}; //the array of link costs
//struct link_change changes[LINKCHANGES]={{10000,0,1,60}, {20000,0,1,4}}; // the array of link changes
   ...
   while (1) {
	    ...
	    printf(" src:%2d,",eventptr->rtpktptr->sourceid);
            printf(" dest:%2d,",eventptr->rtpktptr->destid);
             printf(" contents:");
            for( i=0;i<NUMOFNODES;i++)
            	printf(" %3d", eventptr->rtpktptr->mincost[i]);
            printf("\n");
            }
          }
        clocktime = eventptr->evtime;    /* update time to next event time */
        if (eventptr->evtype == FROM_LAYER2 ) {
        	recv_update(&nodes[eventptr->eventity], eventptr->rtpktptr);
         }
        else if (eventptr->evtype == LINK_CHANGE ) {
        	for( i=0;i<LINKCHANGES;i++)
        	{
        		if(clocktime==changes[i].time)
        		{
        			link_change_handler(&nodes[changes[i].node_a],changes[i].node_b,changes[i].new_cost);
        			link_change_handler(&nodes[changes[i].node_b],changes[i].node_a,changes[i].new_cost);
        			break;
        		}
        	}	
	  }
...
void init()                         /* initialize the simulator */
{
  ...
  struct event *evptr;  
   ...
   for (i=0;i<NUMOFNODES;i++)
   	node_init(connectcosts[i], i, &nodes[i]);
